...
int elka_ctl_loop(int argc, char *argv[]) {
  ...
  */
  ...
  // same
  input_rc_trim.values[0] = RAW_ROLL_BASELINE;
  input_rc_trim.values[1] = RAW_ROLL_BASELINE;
  input_rc_trim.values[SPEKTRUM_THRUST_CHANNEL] = RAW_THRUST_BASELINE;
  input_rc_trim.values[3] = RAW_ROLL_BASELINE;
  input_rc.values[0] = RAW_ROLL_BASELINE;
  input_rc.values[1] = RAW_ROLL_BASELINE;
  ...
  while (!thread_should_exit) {
    ...
    if (poll_ret == 0) {
      ...
      // Kill motors and reset setpoints for safety
      nav->reset_setpoints();
      ...
      if (_serial_state == SERIAL_STATE_KILL ||
        ...
        // Kill motors and reset setpoints for safety
        //TODO put reset back in but make it so that controller being on doesn't
        //mess with control flow
        //nav->reset_setpoints();
        msg_type=MSG_TYPE_KILL;
        if (check_state(msg_type) &&
            (pack_spektrum_cmd(
...
int pack_spektrum_cmd(elka_packet_s *snd,
        ...
        PX4_INFO("msg type: %d, channel values\n0:\t%" PRIu16 "\n1:\t%" PRIu16 "\n2:\t"
"%" PRIu16 "\n3:\t%" PRIu16 "\n4:\t%" PRIu16 "\n5:\t%" PRIu16 "\n6:\t"
"%" PRIu16 "\n7:\t%" PRIu16 "\n8:\t%" PRIu16 "\n9:\t%" PRIu16 "\n10:\t"
"%" PRIu16 "",
            msg_type,
...
void serial_state_timeout_check(void *arg) {
  ...
  timeout_duration = hrt_abstime(300000); // 0.1s
