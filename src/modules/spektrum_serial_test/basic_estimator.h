...
struct pose_stamped_s {
  ...
  // `hold_until_time` flag useful for setpoints that should hold for the entire
  // prescribed duration but not be held indefinitely
  // This is so certain setpoints (like warmup) can't be thrown out if the
  // drone is at the prescribed setpoint location.
  bool error_pose=false,initialized,timeout,hold,hold_until_time,land;
  ...
  pose_stamped_s(hrt_abstime tau,
    ...
    hold_until_time=param_mask&SETPOINT_PARAM_HOLD_UNTIL_TIME;
